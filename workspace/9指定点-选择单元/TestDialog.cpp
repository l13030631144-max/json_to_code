...
void TestDialog::initialize_cb()
{
    try
    {
        ...
        selectPart_part = dynamic_cast<PWOpen::BlockStyler::SelectPartFromList*>(theDialog->TopBlock()->FindBlock("selectPart0"));
        selectNodes_nodes = dynamic_cast<PWOpen::BlockStyler::SelectNode*>(theDialog->TopBlock()->FindBlock("selectNodes0"));
        selectElements_elements = dynamic_cast<PWOpen::BlockStyler::SelectElement*>(theDialog->TopBlock()->FindBlock("selectElements0"));
    }
    catch(std::exception& ex)
    {
        TestDialog::theUI->PWMessageBox()->Show("Block Styler", PWOpen::PWMessageBox::DialogTypeError, ex.what());
...
int TestDialog::update_cb(PWOpen::BlockStyler::UIBlock* block)
{
    try
    {
        if (block == specifyPoint_origin)
        {
        }
        else if (block == superPoint_point)
        {
        }
        ...
        else if (block == selectPart_part)
        {
        }
        else if (block == selectNodes_nodes)
        {
        }
        else if (block == selectElements_elements)
        {
        }
    }
    catch(std::exception& ex)
