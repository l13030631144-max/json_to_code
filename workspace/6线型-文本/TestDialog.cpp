...
void TestDialog::initialize_cb()
{
    try
    {
        lineFont_style = dynamic_cast<PWOpen::BlockStyler::LineFont*>(theDialog->TopBlock()->FindBlock("lineFont0"));
        lineWidth_width = dynamic_cast<PWOpen::BlockStyler::LineWidth*>(theDialog->TopBlock()->FindBlock("lineWidth0"));
        lineColorFontWidth_line = dynamic_cast<PWOpen::BlockStyler::LineColorFontWidth*>(theDialog->TopBlock()->FindBlock("lineColorFontWidth0"));
        textColorFontWidth_text = dynamic_cast<PWOpen::BlockStyler::TextColorFontWidth*>(theDialog->TopBlock()->FindBlock("textColorFontWidth0"));
    }
    catch(std::exception& ex)
    {
        TestDialog::theUI->PWMessageBox()->Show("Block Styler", PWOpen::PWMessageBox::DialogTypeError, ex.what());
...
int TestDialog::update_cb(PWOpen::BlockStyler::UIBlock* block)
{
    try
    {
        if (block == lineFont_style)
        {
        }
        else if (block == lineWidth_width)
        {
        }
        else if (block == lineColorFontWidth_line)
        {
        }
        else if (block == textColorFontWidth_text)
        {
        }
    }
    catch(std::exception& ex)
