...
void TestDialog::initialize_cb()
{
    try
    {
        separator_main = dynamic_cast<PWOpen::BlockStyler::Separator*>(theDialog->TopBlock()->FindBlock("separator0"));
        colorPicker_objectColor = dynamic_cast<PWOpen::BlockStyler::ObjectColorPicker*>(theDialog->TopBlock()->FindBlock("colorPicker0"));
        rgbColorPicker_color = dynamic_cast<PWOpen::BlockStyler::RGBColorPicker*>(theDialog->TopBlock()->FindBlock("rGBColorPicker0"));
        drawingArea_preview = dynamic_cast<PWOpen::BlockStyler::DrawingArea*>(theDialog->TopBlock()->FindBlock("drawingArea0"));
        blockLayer_layer = dynamic_cast<PWOpen::BlockStyler::LayerBlock*>(theDialog->TopBlock()->FindBlock("layer0"));
    }
    catch(std::exception& ex)
    {
        TestDialog::theUI->PWMessageBox()->Show("Block Styler", PWOpen::PWMessageBox::DialogTypeError, ex.what());
...
int TestDialog::update_cb(PWOpen::BlockStyler::UIBlock* block)
{
    try
    {
        if (block == separator_main)
        {
        }
        else if (block == colorPicker_objectColor)
        {
        }
        else if (block == rgbColorPicker_color)
        {
        }
        else if (block == drawingArea_preview)
        {
        }
        else if (block == blockLayer_layer)
        {
        }
    }
    catch(std::exception& ex)
