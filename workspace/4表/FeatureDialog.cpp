...
void FeatureDialog::initialize_cb()
{
    try
    {
        table_table0 = dynamic_cast<PWOpen::BlockStyler::Table*>(theDialog->TopBlock()->FindBlock("table_table0"));
        blockLabel_label0 = dynamic_cast<PWOpen::BlockStyler::Label*>(theDialog->TopBlock()->FindBlock("blockLabel_label0"));
    }
    catch(std::exception& ex)
    {
        FeatureDialog::theUI->PWMessageBox()->Show("Block Styler", PWOpen::PWMessageBox::DialogTypeError, ex.what());
