...
void TestDialog::initialize_cb()
{
    try
    {
        blockLabel_info = dynamic_cast<PWOpen::BlockStyler::Label*>(theDialog->TopBlock()->FindBlock("label0"));
        blockToggle_switch = dynamic_cast<PWOpen::BlockStyler::Toggle*>(theDialog->TopBlock()->FindBlock("toggle0"));
    }
    catch(std::exception& ex)
    {
        TestDialog::theUI->PWMessageBox()->Show("Block Styler", PWOpen::PWMessageBox::DialogTypeError, ex.what());
...
int TestDialog::update_cb(PWOpen::BlockStyler::UIBlock* block)
{
    try
    {
        if (block == blockLabel_info)
        {
        }
        else if (block == blockToggle_switch)
        {
        }
    }
    catch(std::exception& ex)
