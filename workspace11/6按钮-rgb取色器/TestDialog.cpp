...
void TestDialog::initialize_cb()
{
    try
    {
        blockButton_button      = dynamic_cast<PWOpen::BlockStyler::Button*>(theDialog->TopBlock()->FindBlock("blockButton_button"));
        listBox_list            = theDialog->TopBlock()->FindBlock("listBox_list");
        separator_main          = dynamic_cast<PWOpen::BlockStyler::Separator*>(theDialog->TopBlock()->FindBlock("separator_main"));
        colorPicker_objectColor = dynamic_cast<PWOpen::BlockStyler::ObjectColorPicker*>(theDialog->TopBlock()->FindBlock("colorPicker_objectColor"));
        rgbColorPicker_rgb      = dynamic_cast<PWOpen::BlockStyler::RGBColorPicker*>(theDialog->TopBlock()->FindBlock("rgbColorPicker_rgb"));
    }
    catch(std::exception& ex)
    {
        TestDialog::theUI->PWMessageBox()->Show("Block Styler", PWOpen::PWMessageBox::DialogTypeError, ex.what());
...
int TestDialog::update_cb(PWOpen::BlockStyler::UIBlock* block)
{
    try
    {
        if (block == blockButton_button)
        {
        }
        else if (block == listBox_list)
        {
        }
        ...
        else if (block == rgbColorPicker_rgb)
        {
        }
    }
    catch(std::exception& ex)
