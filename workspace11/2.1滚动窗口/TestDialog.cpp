...
void TestDialog::initialize_cb()
{
    try
    {
        ...
        blockString_string0 = dynamic_cast<PWOpen::BlockStyler::StringBlock*>(theDialog->TopBlock()->FindBlock("blockString_string0"));
        blockButton_button0 = dynamic_cast<PWOpen::BlockStyler::Button*>(theDialog->TopBlock()->FindBlock("blockButton_button0"));
    }
    catch(std::exception& ex)
    {
        TestDialog::theUI->PWMessageBox()->Show("Block Styler", PWOpen::PWMessageBox::DialogTypeError, ex.what());
