...
void TestDialog::initialize_cb()
{
    try
    {
        ...
        specifyCsys_main = dynamic_cast<PWOpen::BlockStyler::SpecifyCSYS*>(theDialog->TopBlock()->FindBlock("specifyCsys_main"));
        cursorLocation_main = dynamic_cast<PWOpen::BlockStyler::SpecifyLocation*>(theDialog->TopBlock()->FindBlock("cursorLocation_main"));
        manipulator_main = dynamic_cast<PWOpen::BlockStyler::SpecifyOrientation*>(theDialog->TopBlock()->FindBlock("manipulator_main"));
        reverseDir_main = dynamic_cast<PWOpen::BlockStyler::ReverseDirection*>(theDialog->TopBlock()->FindBlock("reverseDir_main"));
        setList_main = dynamic_cast<PWOpen::BlockStyler::SetList*>(theDialog->TopBlock()->FindBlock("setList_main"));
    }
    catch(std::exception& ex)
    {
        TestDialog::theUI->PWMessageBox()->Show("Block Styler", PWOpen::PWMessageBox::DialogTypeError, ex.what());
...
int TestDialog::update_cb(PWOpen::BlockStyler::UIBlock* block)
{
    try
    {
        ...
        else if (block == specifyCsys_main)
        {
        }
        else if (block == cursorLocation_main)
        {
        }
        else if (block == manipulator_main)
        {
        }
        else if (block == reverseDir_main)
        {
        }
        else if (block == setList_main)
        {
        }
    }
    catch(std::exception& ex)
