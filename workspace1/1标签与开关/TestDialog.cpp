...
void TestDialog::initialize_cb()
{
    try
    {
        ...
        blockLabel_aaa = dynamic_cast<PWOpen::BlockStyler::Label*>(theDialog->TopBlock()->FindBlock("blockLabel_aaa"));
        blockToggle_bbb = dynamic_cast<PWOpen::BlockStyler::Toggle*>(theDialog->TopBlock()->FindBlock("blockToggle_bbb"));
    }
    catch(std::exception& ex)
    {
        TestDialog::theUI->PWMessageBox()->Show("Block Styler", PWOpen::PWMessageBox::DialogTypeError, ex.what());
...
int TestDialog::update_cb(PWOpen::BlockStyler::UIBlock* block)
{
    try
    {
        ...
        else if (block == blockLabel_aaa)
        {
        }
        else if (block == blockToggle_bbb)
        {
        }
    }
    catch(std::exception& ex)
