...
void TestDialog::initialize_cb()
{
    try
    {
        ...
        listBox_main = theDialog->TopBlock()->FindBlock("listBox_main");
        separator_main = dynamic_cast<PWOpen::BlockStyler::Separator*>(theDialog->TopBlock()->FindBlock("separator_main"));
        objectColorPicker_main = dynamic_cast<PWOpen::BlockStyler::ObjectColorPicker*>(theDialog->TopBlock()->FindBlock("objectColorPicker_main"));
    }
    catch(std::exception& ex)
    {
        TestDialog::theUI->PWMessageBox()->Show("Block Styler", PWOpen::PWMessageBox::DialogTypeError, ex.what());
...
int TestDialog::update_cb(PWOpen::BlockStyler::UIBlock* block)
{
    try
    {
        ...
        else if (block == listBox_main)
        {
        }
        else if (block == separator_main)
        {
        }
        else if (block == objectColorPicker_main)
        {
        }
    }
    catch(std::exception& ex)
