...
int TestDialog::update_cb(PWOpen::BlockStyler::UIBlock* block)
{
    try
    {
        ...
        if (block == enum_main)
        {
            ...
            // blockToggle_main -> enum0(a)
            // blockLabel_main  -> enum0(b)
            // blockButton_main -> enum0(c)
            blockToggle_main->GetProperties()->SetLogical("Show", enumValue == "a");
            blockLabel_main->GetProperties()->SetLogical("Show", enumValue == "b");
            blockButton_main->GetProperties()->SetLogical("Show", enumValue == "c");
        }
