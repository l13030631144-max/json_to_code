...
void TestDialog::initialize_cb()
{
    try
    {
        ...
        blockString_input = dynamic_cast<PWOpen::BlockStyler::StringBlock*>(theDialog->TopBlock()->FindBlock("blockString_input"));
        multilineString_content = theDialog->TopBlock()->FindBlock("multilineString_content");
        blockButton_action = dynamic_cast<PWOpen::BlockStyler::Button*>(theDialog->TopBlock()->FindBlock("blockButton_action"));
    }
    catch(std::exception& ex)
    {
        TestDialog::theUI->PWMessageBox()->Show("Block Styler", PWOpen::PWMessageBox::DialogTypeError, ex.what());
...
int TestDialog::update_cb(PWOpen::BlockStyler::UIBlock* block)
{
    try
    {
        ...
        else if (block == blockString_input)
        {
        }
