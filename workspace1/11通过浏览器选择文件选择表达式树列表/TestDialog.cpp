...
void TestDialog::initialize_cb()
{
    try
    {
        ...
        fileBrowser_main = theDialog->TopBlock()->FindBlock("fileBrowser_main");
        selectExpression_main = theDialog->TopBlock()->FindBlock("selectExpression_main");
        tree_main = dynamic_cast<PWOpen::BlockStyler::Tree*>(theDialog->TopBlock()->FindBlock("tree_main"));
    }
    catch(std::exception& ex)
    {
        TestDialog::theUI->PWMessageBox()->Show("Block Styler", PWOpen::PWMessageBox::DialogTypeError, ex.what());
...
int TestDialog::update_cb(PWOpen::BlockStyler::UIBlock* block)
{
    try
    {
        ...
        else if (block == fileBrowser_main)
        {
        }
        else if (block == selectExpression_main)
        {
        }
        else if (block == tree_main)
        {
        }
    }
    catch(std::exception& ex)
