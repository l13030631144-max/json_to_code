...
void TestDialog::initialize_cb()
{
    try
    {
        blockGroup_noTitle    = dynamic_cast<PWOpen::BlockStyler::Group*>(theDialog->TopBlock()->FindBlock("blockGroup_noTitle"));
        table_tableLayout     = dynamic_cast<PWOpen::BlockStyler::Table*>(theDialog->TopBlock()->FindBlock("table_tableLayout"));
        orientXpress_main     = dynamic_cast<PWOpen::BlockStyler::OrientXpress*>(theDialog->TopBlock()->FindBlock("orientXpress_main"));
        micropositioning_micro = dynamic_cast<PWOpen::BlockStyler::Microposition*>(theDialog->TopBlock()->FindBlock("micropositioning_micro"));
    }
    catch(std::exception& ex)
    {
        TestDialog::theUI->PWMessageBox()->Show("Block Styler", PWOpen::PWMessageBox::DialogTypeError, ex.what());
...
int TestDialog::update_cb(PWOpen::BlockStyler::UIBlock* block)
{
    try
    {
        if (block == blockGroup_noTitle)
        {
        }
        else if (block == table_tableLayout)
        {
        }
        else if (block == orientXpress_main)
        {
        }
        else if (block == micropositioning_micro)
        {
        }
    }
    catch(std::exception& ex)
